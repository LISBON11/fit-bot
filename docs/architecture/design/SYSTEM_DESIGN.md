...
graph TB
    subgraph "Клиенты"
        TG["📱 Telegram Bot"]
        APP["📲 Мобильное приложение<br/>(будущее)"]
    end

    subgraph "API Gateway (будущее)"
        GW["🔀 API Gateway / BFF"]
    end

    subgraph "Core Services (монолит на MVP)"
        BOT["🤖 Bot Handler<br/>Telegraf + FSM"]
        STT["🎙️ STT Service<br/>Deepgram Nova-3"]
        NLU["🧠 NLU Parser<br/>Workout Parser"]
        WRK["💪 Workout Service<br/>CRUD + Draft flow"]
        EXR["🏋️ Exercise Registry<br/>Синонимы + ID"]
        USR["👤 User Service<br/>Auth + Profile"]
        PUB["📣 Publisher<br/>Канал / Чат"]
    end

    subgraph "Данные"
        PG["🐘 PostgreSQL"]
        REDIS["⚡ Redis<br/>Sessions + Cache"]
        S3["📦 Object Storage<br/>Аудио (опц.)"]
    end
...
| **STT Service**       | Скачивание голосового → транскрипция (OGG Opus нативно)                                       | Node.js/TS (вызов Deepgram Nova-3 API) |
| **NLU Parser**        | Извлечение структурированных данных из текста (дата, фокус, упражнения, подходы, комментарии) | Node.js/TS (+ DeepSeek V3 для NLU)     |
...
sequenceDiagram
    ...
    participant CH as Чат/Канал

    U->>TG: 🎤 Голосовое сообщение
    TG->>BOT: Update (voice)
    BOT->>U: ⏳ "Обрабатываю..."
    BOT->>STT: Скачать файл + транскрибировать
    STT->>STT: Скачать .oga → Deepgram Nova-3 API (OGG Opus нативно)
    STT-->>BOT: Текст транскрипции
    BOT->>NLU: Распарсить текст
    NLU->>NLU: Извлечь дату, фокус, место, упражнения, подходы, комментарии

    alt Неоднозначное упражнение
        NLU-->>BOT: Нужно уточнение
        BOT->>U: 🔘 Inline-кнопки: "Что именно?"
        U->>BOT: Выбор кнопки
        BOT->>NLU: Продолжить с выбором
    end

    NLU-->>BOT: Структурированный WorkoutDraft
    BOT->>WRK: Сохранить черновик
    WRK->>DB: INSERT workout (status=draft)
    BOT->>U: 📝 Превью + кнопки [✅ Approve] [✏️ Edit] [❌ Cancel]

    alt ✅ Approve
        U->>BOT: Нажал Approve
        BOT->>WRK: Утвердить тренировку
        WRK->>DB: UPDATE status = approved
        BOT->>PUB: Опубликовать
        PUB->>CH: 📣 Красивый пост тренировки
        BOT->>TG: Удалить исходное голосовое сообщение
        BOT->>U: ✅ "Тренировка опубликована!"
    else ✏️ Edit
        U->>BOT: Нажал Edit
        BOT->>U: "Отправьте текст или голосовое с правками"
        U->>BOT: Текст/голос с правками
        BOT->>NLU: Распарсить правки
        NLU-->>BOT: Дельта изменений
        BOT->>WRK: Обновить черновик
        BOT->>U: 📝 Обновлённое превью + кнопки
    else ❌ Cancel
        U->>BOT: Нажал Cancel
        BOT->>WRK: Удалить черновик
        WRK->>DB: DELETE workout
        BOT->>TG: Удалить исходное голосовое
        BOT->>U: "Тренировка отменена"
    end
```
...
```mermaid
graph TD
    A[Получили голосовое] --> B{Скачать файл}
    B -->|Ошибка| E1["⚠️ 'Не смог скачать файл, попробуй ещё раз'"]
    B -->|OK| C{STT}
    C -->|Ошибка| E2["⚠️ 'Не удалось распознать речь, попробуй медленнее или текстом'"]
    C -->|Пустой текст| E3["⚠️ 'Не удалось распознать слова, попробуй снова'"]
    C -->|OK| D{NLU}
    D -->|Ошибка парсинга| E4["⚠️ 'Не смог понять тренировку, вот что я услышал: ...'<br/>Показываем raw text"]
    D -->|Частичный результат| E5["📝 Показываем что распознали + просим уточнить"]
    D -->|OK| F[Показать превью]
```
...
| -------------------------------- | ------------------------------------------------------------------- | ---------------------------------------- | ------------ |
| **Deepgram Nova-3 API** (облако) | Дешевле ($0.004/мин), русский, OGG нативно, $200 кредит, без ffmpeg | Зависимость от сети, американский сервис | ✅ **MVP**   |
| **faster-whisper** (self-hosted) | Бесплатно, CPU-friendly, без зависимостей от внешних API            | Docker, нужно обслуживать                | Альтернатива |
| **OpenAI Whisper API** (облако)  | Отличное качество русского                                          | Платный ($0.006/мин), нужен ffmpeg       | Устарело     |
...
interface SttService {
   ...
   * @param audioBuffer — буфер аудио (OGG Opus и другие форматы)
...
// Реализация через Deepgram Nova-3 API
// OGG Opus (формат Telegram) принимается нативно — ffmpeg не нужен
class DeepgramStt implements SttService {
  async transcribe(audioBuffer: Buffer, language = 'ru'): Promise<string> {
    // 1. Отправляем буфер напрямую в Deepgram API (OGG Opus поддерживается)
    // 2. Возвращаем result.results.channels[0].alternatives[0].transcript
  }
}
```
...
- На MVP используем **Deepgram Nova-3 API** (облачный) — это просто HTTP-запрос, Python не нужен
- Интеграция через `@deepgram/sdk` npm-пакет — нативно в Node.js
- **ffmpeg не нужен** — Deepgram принимает OGG Opus напрямую
- Python-микросервис с self-hosted моделью понадобится только при:
  - Желании сэкономить при большом объёме
...
Вместо ручного regex/rule-based парсинга используем **DeepSeek V3** c **JSON mode** для извлечения данных:

| Подход                       | Плюсы                                                        | Минусы                                            |
| ---------------------------- | ------------------------------------------------------------ | ------------------------------------------------- |
| **LLM (DeepSeek V3) JSON**   | Понимает контекст, синонимы, неформальную речь, «из коробки» | Платный, зависимость от API, задержка             |
...
> [!TIP]
> **Рекомендация:** На MVP — DeepSeek V3 (deepseek-chat). JSON mode, отличный русский, дешевле GPT-4o-mini, доступен с РФ IP-адресов.

#### Промпт и JSON-схема для парсинга

```typescript
// Схема ожидаемого результата парсинга (соответствует ParsedWorkoutSchema Zod)
...
При вызове DeepSeek V3 передаём:
...
```mermaid
graph TD
    A[NLU распознал 'присед'] --> B{Есть в справочнике<br/>пользователя?}
    B -->|Да, однозначно| C[Маппим на canonical exercise]
    B -->|Нет или неоднозначно| D{Есть варианты?}
    D -->|2+ варианта| E["🔘 Inline-кнопки:<br/>1. Back Squat<br/>2. Front Squat<br/>3. Goblet Squat<br/>4. Добавить новое"]
    D -->|0 вариантов| F["🔘 Создать новое упражнение?"]
    E --> G[Пользователь выбрал]
    G --> H[Сохраняем маппинг<br/>'присед' → выбранный ID<br/>для этого пользователя]
```
...
erDiagram
    USERS {
        uuid id PK
        varchar telegram_id UK "Nullable — позже привяжем"
        varchar telegram_username
        varchar email UK "Nullable — для приложения"
        varchar password_hash "Nullable — для приложения"
    ...
    AUTH_PROVIDERS {
        ...
        varchar provider_user_id "уникальный ID провайдера"
        jsonb metadata "доп. данные провайдера"
        timestamp created_at
    }
    ...
    WORKOUTS {
        ...
        text raw_transcript "Исходный текст транскрипции"
        integer source_message_id "Telegram message ID для удаления"
        integer preview_message_id "Telegram message ID превью"
        integer published_message_id "ID в канале/чате"
        timestamp created_at
        timestamp updated_at
    }
    ...
    EXERCISES {
        uuid id PK
        varchar canonical_name UK "Уникальное каноническое имя"
        varchar display_name_ru "Отображаемое имя на русском"
        varchar display_name_en "Отображаемое имя на английском"
        varchar[] muscle_groups "Связанные мышечные группы"
        varchar category "compound | isolation | cardio"
        boolean is_global "true = общий, false = пользовательский"
        uuid created_by FK "Nullable — кто создал"
        timestamp created_at
    }
    ...
    EXERCISE_SYNONYMS {
        uuid id PK
        uuid exercise_id FK
        varchar synonym "Текст синонима (lowercase)"
        varchar language "ru | en"
        boolean is_global "Глобальный или пользовательский"
        uuid user_id FK "Nullable — для пользовательских"
        timestamp created_at
    }
    ...
    USER_EXERCISE_MAPPINGS {
        uuid id PK
        uuid user_id FK
        varchar input_text "Что пользователь сказал"
        uuid exercise_id FK "На что замаплено"
        integer use_count "Сколько раз использовано"
        timestamp created_at
        timestamp updated_at
    }
    ...
    WORKOUT_EXERCISES {
        ...
        uuid workout_id FK
        uuid exercise_id FK
        integer sort_order "Порядок в тренировке"
        varchar raw_name "Как пользователь назвал"
        timestamp created_at
    }
    ...
    WORKOUT_COMMENTS {
        uuid id PK
        uuid workout_id FK
        uuid workout_exercise_id FK "Nullable — может быть общий"
...
> [!NOTE]
> Бизнес-логика привязки мобильного приложения к Telegram (Link Account) и механика прозрачной регистрации описаны в документе с бизнес-требованиями. Ниже приведена только техническая реализация авторизации.
...
stateDiagram-v2
    ...
    Idle --> ProcessingVoice: Получили голосовое
    Idle --> ProcessingText: Получили текст
    Idle --> EditingByDate: /edit + дата

    ProcessingVoice --> WaitingForSTT: Отправили в STT
    WaitingForSTT --> Parsing: Получили текст
    ProcessingText --> Parsing: Парсим текст напрямую

    Parsing --> Disambiguating: Неоднозначное упражнение
    Disambiguating --> Parsing: Пользователь выбрал
    Parsing --> DraftReady: Все упражнения распознаны

    DraftReady --> Idle: ❌ Cancel
    DraftReady --> Editing: ✏️ Edit
    DraftReady --> Publishing: ✅ Approve

    Editing --> ProcessingEditVoice: Голосовое с правками
    Editing --> ProcessingEditText: Текст с правками
    ProcessingEditVoice --> WaitingForSTT
    ProcessingEditText --> Parsing

    EditingByDate --> Editing: Нашли тренировку

    Publishing --> Idle: Опубликовано
```
...
> [!NOTE]
> Пример того, как выглядит превью (UX) и процесс диалогового редактирования (User Journey), описан в сценариях 1 и 3 в [бизнес-требованиях](../../business/general.md). Техническая реализация (стейт-машина) приведена выше.
...
```mermaid
graph TD
    A["Входное слово: 'присед'"] --> B["LOWER + нормализация"]
    B --> C{"Поиск в user_exercise_mappings<br/>(конкретный пользователь)"}
    C -->|Найден| D["✅ Возвращаем exercise_id<br/>use_count++"]
    C -->|Не найден| E{"Поиск в exercise_synonyms<br/>(user_id = current OR is_global)"}
    E -->|1 совпадение| D
    E -->|2+ совпадений| F["⚠️ Неоднозначно → inline-кнопки"]
    E -->|0 совпадений| G{"Нечёткий поиск<br/>(Levenshtein / trigram)"}
    G -->|Есть похожие| F
    G -->|Ничего| H["➕ Предложить создать новое"]
    F -->|Пользователь выбрал| I["Сохраняем в user_exercise_mappings"]
    I --> D
    H -->|Создал| J["Новое упражнение + синоним"]
    J --> D
```
...
```mermaid
graph LR
    subgraph "Этап 1: MVP (1 пользователь)"
        M["🟢 Монолит<br/>Node.js<br/>Bot + STT + NLU<br/>+ Workout + User<br/>+ Publisher"]
        PG1["PostgreSQL"]
        R1["Redis"]
        M --> PG1
        M --> R1
    ...
    subgraph "Этап 2: Приложение (десятки пользователей)"
        BOT2["🤖 Bot Service"]
        API2["🔷 Workout API<br/>(REST)"]
        PG2["PostgreSQL"]
        R2["Redis"]
        APP2["📱 Mobile App"]
        BOT2 --> API2
    ...
    subgraph "Этап 3: Масштаб (сотни+ пользователей)"
        BOT3["🤖 Bot"]
        GW3["🔀 API GW"]
        STT3["🎙️ STT/NLU<br/>Service"]
        API3["🔷 Workout API"]
        AN3["📊 Analytics"]
        Q3["📬 Message Queue"]
...
> [!IMPORTANT]
> **Рекомендация:** На MVP — монолит. STT через Deepgram API и NLU через DeepSeek API (оба — HTTP-вызовы) фактически уже «вынесены» как внешние сервисы. Выносить в отдельный контейнер стоит только при self-hosted моделях.
...
| `DEEPGRAM_API_KEY`   | `.env` → переменная окружения | Аналогично (STT)                                  |
| `DEEPSEEK_API_KEY`   | `.env` → переменная окружения | Аналогично (NLU)                                  |
...
3. Отправляет напрямую в Deepgram Nova-3 API (OGG Opus нативно, ffmpeg не нужен)
...
| **STT**                | Deepgram Nova-3 API               | Русский язык, OGG нативно (без ffmpeg), $0.004/мин, $200 кредит на старт |
| **NLU / Парсинг**      | DeepSeek V3 (deepseek-chat)       | Structured output через JSON mode, отличный русский, дешевле GPT-4o-mini |
| **БД**                 | PostgreSQL 16 (via Supabase)      | Реляционные данные, JSONB, аналитика, надёжность                         |
| **БД Хостинг (prod)**  | **Supabase** (managed PostgreSQL) | Бесплатно до 500 MB, авто-бэкапы, UI, без ручного администрирования      |
...
| **Автоматизация**      | Lefthook                          | Быстрые параллельные Git-хуки (pre-commit, pre-push)                     |
| **Тестирование**       | Jest + jest-mock-extended         | Строгие типизированные мок-тесты, coverage                               |
...
| **Деплой**             | Docker + Docker Compose + GHCR    | Образ в GitHub Container Registry, автодеплой через GitHub Actions       |

### Что на Python?

> [!NOTE]
> **На MVP — ничего.** Deepgram Nova-3 API и GPT API вызываются через HTTP из Node.js.
>
...
Локальная разработка — PostgreSQL и Redis в Docker:
...
> [!NOTE]
> **Production** (`docker-compose.prod.yml`) — PostgreSQL **не запускается в Docker**.
> Используется [Supabase](https://supabase.com) (managed PostgreSQL, бесплатный tier 500 MB).
> На VPS в Docker работают только **bot** и **redis**.
> Подробности: [ADR-0003](../decisions/0003-supabase-managed-postgres.md), [DEPLOYMENT.md](../../infrastructure/DEPLOYMENT.md).
...
| **Деплой** | Запуск VPS, Telegram Bot, миграции и тестирование   | 1 неделя       |
...
- Для REST API (Этап 7.1) используется `@fastify/swagger` для автогенерации спецификации.
