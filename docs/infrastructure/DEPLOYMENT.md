...
GitHub (push → main)
  ├─[CI]──► lint + test + build
  ├─[CI]──► docker build → ghcr.io/lisbon11/fit-tel-bot:latest
...
4. Настройка:
   ...
   - Публичный IP: **Автоматически** (выделится статический)
   - SSH-ключи: вставь публичный ключ:
     ```bash
...
```bash
ssh yc-user@<IP-адрес-VM>
```

> [!NOTE]
> В Яндекс Облаке пользователь по умолчанию — `yc-user`, не `root`.
...
LOG_LEVEL=info
PUBLISH_CHAT_ID=<ID канала для публикаций>
```
...
1. ✅ Lint + tests + build
2. 🐳 Docker build → push `ghcr.io/lisbon11/fit-tel-bot:latest`
