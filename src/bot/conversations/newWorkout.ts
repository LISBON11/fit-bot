...
export async function newWorkout(
  ...
  const draftResult = await parseAndDisambiguateUserInput(conversation, ctx, rawText, 'new');
  ...
  while (true) {
    ...
    if (incomingWorkoutId && incomingWorkoutId !== workoutId) {
      await actionCtx.answerCallbackQuery('⚠️ Эта тренировка уже неактуальна');
      await actionCtx
    ...
    if (action === 'appr') {
      ...
      const editResult = await parseAndDisambiguateUserInput(
        ...
        editRawText,
        'edit',
        JSON.stringify(fullWorkout, null, 2),
        workoutId,
      );
